...
from data_loading import data_classes,data_content
...
def punctationRemoval(words : list[list[str]]):
    chars_to_remove = ['.', ',', '!', '?', ':', ';', "'", '"', '-', '(', ')', '[', ']', '{', '}', '/'] #te znaki usuniemy by łatwiej szkolić model
...
preprocessed_mails = lemmatization(stopWordRemoval(punctationRemoval(toLowerCase(tokenization(data_content))))) #dziala
