class Neuron:
    def __init__(self, input_size, id, learning_rate=0.2, convergence_criteria=0.1):
        self.id = id
    ...
    def train(self,input_data,labels):
        ...
        for i in range(0,epochs):
            ...
            for feature_vector, label in zip(input_data, numerical_labels):
                ...
                """
                if isActive == 1:
                    self.weights = [w + self.learning_rate * error_calc * x for w, x in zip(self.weights, feature_vector)]
                else:
                    self.weights = [w - self.learning_rate * error_calc * x for w, x in zip(self.weights, feature_vector)]
